...
make.ecc.datafile.w.cont.mixture <- function(Ndata, ratetrue, sigmaetrue, sigmahobs, sigmakobs, data.to.add = list(), modelname="1", dataname="RC", Nrealizations=1, inits=NULL)
{
 for(i in 1:Nrealizations) {
  hktest <- generate.ecc.mixture.cont(Ndata,ratetrue,sigmaetrue,sigmahobs,sigmakobs)
  data.base <- list(H_OBS=hktest$hhat, K_OBS=hktest$khat, H_SIGMA=rep(sigmahobs,Ndata), K_SIGMA=rep(sigmahobs,Ndata), H_TRUE=hktest$htrue, K_TRUE=hktest$ktrue, P=hktest$P )
  filename <- paste("../datasets/hk_",modelname,"_",dataname,"_",i,".dat",sep='') 
  write.table( data.base , file=filename, sep=' ',row.names=FALSE)
  }
...
make.datasets <- function() {
 ...
 make.ecc.datafile.w.cont.mixture(500,2.0, 0.2, 0.001, 0.001, modelname="RC", dataname="best", Nrealizations=20)
 #hist(tmp$htrue,breaks=40)
 #hist(tmp$ktrue,breaks=40)
 #hist(tmp$hhat,breaks=40)
 #hist(tmp$khat,breaks=40)
...
#
 ...
 make.ecc.datafile.w.disc.mixture(50,c(0.6,0.3,0.1), c(0.05,0.2,0.5), 0.001, 0.001, data.to.add=list(Ncomp=3), modelname="R3", dataname="better", Nrealizations=20)
  make.ecc.datafile.w.cont.mixture(50,2.0, 0.2, 0.001, 0.001, modelname="RC", dataname="better", Nrealizations=20)
...
#
 ...
 make.ecc.datafile.w.cont.mixture(60,2.0, 0.2, 0.04, 0.08, modelname="RC", dataname="occ", Nrealizations=20)
...
#
 ...
 make.ecc.datafile.w.cont.mixture(90,2.0, 0.2, 0.25, 0.12, modelname="RC", dataname="ast", Nrealizations=20)
